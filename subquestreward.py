...
class SubquestReward(db.Model):
    ...
    reward_type = db.Column(db.String(50), nullable=False)  # xp, role, custom, token
    distribution_type = db.Column(db.String(50), nullable=False, default="ALL")  # ALL, FCFS, raffle, vote
    reward_data = db.Column(db.Text, nullable=True)  # JSON for extra info (network, contract, amount, role_id, etc.)
