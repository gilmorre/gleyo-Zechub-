...
//   banner.style.transition = 'opacity .25s ease, transform .25s ease, height .25s ease, margin .25s ease';
//   banner.style.opacity = '0';
//   banner.style.transform = 'translateY(-8px)';
//   banner.style.height = banner.offsetHeight + 'px'; // lock height first
...
//   /* remove sheller AFTER animation starts */
//   setTimeout(() => {
//     shell.classList.remove('has-sheller');
//     updateShellHeight();
//   }, 150);
...
//   /* if shell does NOT have sheller → reset */
//   if (!shell.classList.contains('has-sheller') || !banner) {
//     shell.style.height = '';
//     return;
//   }
...
//   let extraOffset = window.innerWidth <= 767 ? 80 : 33;

//   const totalSubtract = bannerHeight + extraOffset;

//   shell.style.height = `calc(100dvh - ${totalSubtract}px)`;
// }


// updateShellHeight();
// /* run on load */
// window.addEventListener('load', updateShellHeight);
...
//   const metrics = ["total","active","new"];

//   const maxValue = Math.max(
//     ...data.flatMap(d => metrics.map(m => d[m]))
//   ) * 1.15;
...
//   const xLabels = svg.querySelector(".x-labels");
//   const yLabels = svg.querySelector(".y-labels");
...
//     const y = padding + (i * (height - padding * 2) / steps);
//     const value = Math.round(maxValue - (i / steps) * maxValue);
...
//     yLabels.innerHTML += `
//       <text x="5" y="${y + 4}" class="axis-text">${value}</text>
//     `;
//   }
...
//         const prevX = scaleX(i - 1);
//         const prevY = scaleY(data[i - 1][metric]);
...
//   const lines = [
//     { key:"total", class:"line-total" },
//     { key:"active", class:"line-active" },
//     { key:"new", class:"line-new" }
//   ];
...
//   pointsGroup.setAttribute("class","chart-point");
...
//     path.setAttribute("d", buildPath(l.key));
//     path.setAttribute("class", `line-path ${l.class}`);
//     path.dataset.metric = l.key;

//     linesG.appendChild(path);

//     /* create circle for hover */
//     const circle = document.createElementNS(
//       "http://www.w3.org/2000/svg",
//       "circle"
//     );

//     circle.setAttribute("class", `point-${l.key}`);
...
//   function showAtIndex(index, clientX, clientY){
...
//       const metric = lines[i].key;

//       const cx = scaleX(index);
//       const cy = scaleY(d[metric]);
...
//     <div class="tt-row">
//         <span class="tt-dot total"></span>
//         <span class="tt-name">Total</span>
//         <span class="tt-value">${d.total.toLocaleString()}</span>
//     </div>

//     <div class="tt-row">
//         <span class="tt-dot active"></span>
//         <span class="tt-name">Active</span>
//         <span class="tt-value">${d.active.toLocaleString()}</span>
//     </div>

//     <div class="tt-row">
//         <span class="tt-dot new"></span>
//         <span class="tt-name">New</span>
//         <span class="tt-value">${d.new.toLocaleString()}</span>
//     </div>
//     `;
...
//     if (left + tipWidth > containerWidth) {
//     left = clientX - containerRect.left - tipWidth - 12;
//     }
...
//     // if overflow top → drop below cursor
//     if (top < 0) {
...
//     const rawIndex = (x - padding) / xStep;

//     const index = Math.floor(rawIndex);
//     const nextIndex = Math.min(index + 1, data.length - 1);

//     const progress = rawIndex - index; // 0 → 1 between points

//     showInterpolated(index, nextIndex, progress, e.clientX, e.clientY);

//     });

//     function smartPositionTooltip(clientX, clientY){
...
//     function showInterpolated(i1, i2, t, clientX, clientY){
...
//     // smooth values
//     const values = {
//         total: Math.round(lerp(d1.total, d2.total, t)),
//         active: Math.round(lerp(d1.active, d2.active, t)),
//         new: Math.round(lerp(d1.new, d2.new, t))
//     };

//     // label only changes when crossing midpoint
//     const label = t < 1 ? d1.label : d2.label;

//     const circles = pointsGroup.querySelectorAll("circle");

//     circles.forEach((circle, idx) => {

//         const metric = lines[idx].key;
...
//         const cx = lerp(scaleX(i1), scaleX(i2), t);
//         const cy = scaleY(v);
...
//         <span class="tt-dot total"></span>
//         <span class="tt-name">Total</span>
//         <span class="tt-value">${values.total.toLocaleString()}</span>
//         </div>

//         <div class="tt-row">
//         <span class="tt-dot active"></span>
//         <span class="tt-name">Active</span>
//         <span class="tt-value">${values.active.toLocaleString()}</span>
//         </div>

//         <div class="tt-row">
//         <span class="tt-dot new"></span>
//         <span class="tt-name">New</span>
//         <span class="tt-value">${values.new.toLocaleString()}</span>
//         </div>
//     `;

//     smartPositionTooltip(clientX, clientY);
//         }
...
//       showAtIndex(index, e.clientX, e.clientY);
//     }
...
//         "7d": [
//         { label:"Mon", total:1200, active:300, new:120 },
//         { label:"Tue", total:1400, active:380, new:150 },
//         { label:"Wed", total:1600, active:420, new:180 },
//         { label:"Thu", total:1900, active:500, new:210 },
//         { label:"Fri", total:2300, active:650, new:260 },
//         { label:"Sat", total:2800, active:800, new:320 },
//         { label:"Sun", total:3400, active:950, new:400 }
//         ],

//         "30d": [
//         { label:"W1", total:5000, active:1200, new:600 },
//         { label:"W2", total:6200, active:1500, new:700 },
//         { label:"W3", total:7800, active:1800, new:850 },
//         { label:"W4", total:9200, active:2100, new:900 }
//         ]

//     };
    

//     return datasets[range] || datasets["7d"];
//     }
...
//     const totalGrowth = percent(last.total, prev.total);
//     const activeGrowth = percent(last.active, prev.active);
//     const newGrowth = percent(last.new, prev.new);

//     document.getElementById("kpiTotal").textContent =
//         last.total.toLocaleString();

//     document.getElementById("kpiActive").textContent =
//         last.active.toLocaleString();

//     document.getElementById("kpiNew").textContent =
//         last.new.toLocaleString();

//     document.getElementById("kpiTotalTrend").textContent =
//         `↑ ${totalGrowth}%`;

//     document.getElementById("kpiActiveTrend").textContent =
//         `↑ ${activeGrowth}%`;

//     document.getElementById("kpiNewTrend").textContent =
//         `↑ ${newGrowth}%`;
//     }
...
//     const data = getAnalyticsData(ANALYTICS_STATE.range);

//     ANALYTICS_STATE.data = data;
